...
export class WorkerPool {
    ...
    public static onWorkerMessage(message: WorkerMessage) {
        ...
        const queueItem = WorkerPool._queue.find(q => q.queueId === message.data.queueId)
        if(!queueItem) {
        ...
        WorkerPool._queue = WorkerPool._queue.filter(q => q.queueId !== message.data.queueId)
    ...
    private static _queueRunner() {
        ...
        const workerId = Object.keys(WorkerPool._pool).find(k => WorkerPool._pool[k].status === WorkerStatusEnum.IDLE)
        ...
        // Send it the message
        WorkerPool._pool[workerId].worker.postMessage({ command: nextMessage.command, payload: nextMessage.payload, queueId: nextMessage.queueId })
    }

    public static run(command: Commands, payload: any, callback: (reply: WorkerReply) => void) {
