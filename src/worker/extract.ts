import { WorkerMessageTypesEnum, type WorkerMessageTypes } from "./messagetypes.ts"
 ...
 * @param length Field width in bytes
 */
const readOctal = (bytes: Uint8Array, offset: number, length: number): number => {
 ...
 */
const padToBlock = (size: number): number => Math.ceil(size / BLOCK_SIZE) * BLOCK_SIZE
 ...
 */
const unpack = async (onProgress: (percent: number) => void): Promise<number> => {
    ...
    while (offset + BLOCK_SIZE <= bytes.length) {
        ...
        if (header.type === TYPE_PAX_NEXT || header.type === TYPE_PAX_GLOBAL) {
            const records = parsePaxRecords(bytes.subarray(dataOffset, dataOffset + header.size))
        ...
        const name = pendingPax?.get("path") ?? pendingName ?? globalPax.get("path") ?? header.name
        ...
        if (segments) {
            if (header.type === TYPE_DIRECTORY) {
                ...
                const directory = await ensureDirectory(root, segments.slice(0, -1), directories)
                // A subarray is a view, so this costs nothing on top of the
                // archive already in memory.
                await writeFile(directory, fileName, bytes.subarray(dataOffset, dataOffset + size))
                extracted++
            }
        }
 ...
 */
export const extractArchive = async (workerData: { id: string; queueId: string }) => {
    ...
    try {
        await unpack((percent) => sendMessage(percent, WorkerMessageTypesEnum.PROGRESS))
